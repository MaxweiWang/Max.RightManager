...
namespace Max.Common.Admin
{
    ...
    public static class AdminStartupConfigure
    {
        ...
        public static void AdminConfigureServices(this IServiceCollection services, IConfiguration Configuration)
        {
            ...
            {
                //注册Models
                Max.Entities.Class.EntitySet.Register(tabs);
            });
            //将DbContextSqlServer对象注册Logic层静态对象中
            Max.Service.Class.AppBase.db = new DbContextSqlServer();
            ...
            services.AddSwaggerGen(options =>
            {
                ...
                var xmlPath = Path.Combine(System.AppContext.BaseDirectory, "Max.Common.Admin.xml");
        ...
        public static void AdminConfigure(this IApplicationBuilder app, IWebHostEnvironment env, IHostApplicationLifetime applicationLifetime)
        {
            ...
            Max.Utilities.HttpContextService.HttpContextHelper.Configure(app.ApplicationServices.GetRequiredService<IHttpContextAccessor>());
            #endregion
